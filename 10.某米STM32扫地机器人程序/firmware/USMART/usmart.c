...
{
#if USMART_ENTIMX_SCAN==1
    xTaskCreate( vUsmartTimerTask, "vUsmartTimerTask", 200, NULL, 2, NULL );    
#endif
...
{
	u8 sta,len;  
	if(ucUSART1_ReceiveMessageNumber)//串口接收完成？
	{					   
		len=ucUSART1ReceiveBufferLength[ucUSART1ReadBufferIndex];	//得到此次接收到的数据长度
		ucUSART1ReceiveBuffer[ucUSART1ReadBufferIndex][len]='\0';	//在末尾加入结束符.        
		sta=usmart_dev.cmd_rec(ucUSART1ReceiveBuffer[ucUSART1ReadBufferIndex]);//得到函数各个信息
		...
		USART1ReceiveDataReadDone(  );//状态寄存器清空	    
	}
}
